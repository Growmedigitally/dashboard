...
import IconsElements from '@template/imageEditor/tabsComposer/graphics/iconsElements';
import GraphicsElements from '@template/imageEditor/tabsComposer/graphics/graphicsElements';
import GraphicsCss from '@template/imageEditor/tabsComposer/graphics/graphics.module.scss'
...
const Patterns = ({ activeObject, rerenderCanvas, canvas, activeObjectsState }: pageProps) => {
    ...
    const TAB_ITEMS_LIST = [
        ...
        },
        {
            key: TAB_TYPES.ICONS,
            icon: <TbIcons />,
            children: <IconsElements onSelect={(imageData) => addPatternImage(imageData)} canvas={canvas} rerenderCanvas={rerenderCanvas} />
        },
        {
            key: TAB_TYPES.GRAPHICS,
            icon: <GiThreeLeaves />,
            children: <GraphicsElements onSelect={(imageData) => addPatternImage(imageData)} canvas={canvas} rerenderCanvas={rerenderCanvas} />
        },
        // {
    ...
    const addPatternImage = (image) => {
        ...
        if (activeObject) {
            ...
            canvas.requestRenderAll();
            setTimeout(() => rerenderCanvas(canvas), 1000);
        }
    }
